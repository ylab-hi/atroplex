...
sample_info sample_manifest::parse_row(
    ...
    // Note: the legacy `expression_attribute` column is no longer parsed.
    // Expression filtering moved to CLI (--min-X / --filter-precedence)
    // and the .qtx sidecar stores every value the GFF row carries. A
    // column with this name in an old manifest is silently ignored.
