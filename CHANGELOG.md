...
### Changed
- **Reuse `create_segment` candidates in mono-exon and reverse absorption** ([#82](https://github.com/ylab-hi/atroplex/pull/82), closes [#42](https://github.com/ylab-hi/atroplex/issues/42)): `classify_mono_exon` and `try_reverse_absorption` previously each issued their own `grove.intersect()` and re-walked overlapping segments' exon chains via `walk_exon_chain`, even though `create_segment` had already gathered the same `(key, chain)` pairs in its `vector<spatial_candidate>` at the top of the function. Both helpers now take that vector and iterate it directly. Net effect per insertion: 2 fewer `grove.intersect()` calls and N fewer chain walks (where N = candidates per locus). No observable behavior change. `try_reverse_absorption`, `classify_mono_exon`, and the `mono_exon_class` enum move from public to private alongside the signature change — `spatial_candidate` is private and grep-verified to have no external callers.
- **Splicing hub threshold semantics** ([#81](https://github.com/ylab-hi/atroplex/pull/81), partial [#61](https://github.com/ylab-hi/atroplex/issues/61)): the hub qualification check moved from `unique_targets > MIN_HUB_BRANCHES` to `>= MIN_HUB_BRANCHES`, so an exon with exactly 10 distinct downstream targets now registers as a hub. The constant's name (`MIN_HUB_BRANCHES = 10`) read as "minimum to qualify" but the check was strict; the new comparison aligns code and intent. The constant is renamed `DEFAULT_HUB_MIN_BRANCHES` and the threshold is now a settable per-instance member. README updated to "≥10".
- **`atroplex export` conservation now matches `inspect`** ([#80](https://github.com/ylab-hi/atroplex/pull/80), closes [#74](https://github.com/ylab-hi/atroplex/issues/74)): adds `--conserved-fraction <float in (0,1]>` to `export`, mirroring the inspect flag from PR #73. `--conserved-only` now respects the configured fraction (default `1.0` = strict). The conservation denominator also changes to count only `type == "sample"` registry entries (annotations excluded), correcting a latent inconsistency where `--conserved-only` previously required segments to be present in annotation entries on indices that have them — segments universal across samples but absent from annotations were silently filtered out. This is a behavior change for `--conserved-only` on indices that contain annotations.

### Added
- **`--min-hub-branches <N>` for inspect** ([#81](https://github.com/ylab-hi/atroplex/pull/81), partial [#61](https://github.com/ylab-hi/atroplex/issues/61)): the splicing-hub branch threshold is now configurable from the CLI. Default `10`, validated `>= 2`, plumbed via `analysis_report::set_hub_min_branches()`. Raise to narrow the catalog when hubs are abundant on cohort-scale builds, lower to surface less-branched events on small indices.
- **Negative-strand absorption test coverage** ([#81](https://github.com/ylab-hi/atroplex/pull/81), partial [#61](https://github.com/ylab-hi/atroplex/issues/61)): all 19 prior absorption fixtures used the `+` strand. New `rule0_fsm_neg_strand.gtf` and `rule7_intron_retention_neg_strand.gtf` exercise FSM merge and mono-exon intron retention on `-` strand. Rules 1–5 are deliberately not duplicated yet because their 5'/3' subsequence labels work off the genomic-coordinate-ordered exon chain, which is inverted relative to transcript orientation on `-` strand — that asymmetry is a separate fix.
