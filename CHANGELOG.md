...
- **Performance: two-level splice site lookup, ostringstream removal** ([#71](https://github.com/ylab-hi/atroplex/pull/71), closes [#15](https://github.com/ylab-hi/atroplex/issues/15), partial [#12](https://github.com/ylab-hi/atroplex/issues/12)): splice sites use `map<string, set<size_t>>` instead of flat `set<splice_site>` — eliminates per-lookup string copies and ~10-20 MB redundant storage. Signature methods use `string::reserve` + `+=` instead of `ostringstream`.
